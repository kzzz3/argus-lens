...
- [x] P2 LocalSessionStore delegation slice: keep `SessionRepository` behavior unchanged while splitting local session identity and credential persistence behind separate package-internal stores.
...
- [x] P2: continue splitting session/token/crypto responsibilities and make repository/data-source/use-case boundaries explicit before Gradle extraction; `LocalSessionStore` now delegates identity and credentials to separate stores, but crypto/key operations remain in the credential store implementation; first UseCase work is limited to real multi-repository outgoing chat/media orchestration.
