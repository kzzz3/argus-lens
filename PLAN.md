...
- [ ] P2: continue splitting session/token/crypto responsibilities and make repository/data-source/use-case boundaries explicit before Gradle extraction; `LocalSessionStore` now delegates identity and credentials to separate stores, but crypto/key operations remain in the credential store implementation.
