...
| 2026-04-28 | P3 process-death restore TDD | Red `AppInitialHydrationRuntimeTest.hydrate_withRestoredChatEntryAndMatchingConversationRoutesToChatAfterThreadsLoad` failed on missing safe restore API; red `hydrate_withoutInitialTokenClearsRestorableEntryAndDoesNotRoute` failed until unsafe saved chat context cleanup was added; Oracle PASS, then red/green hardening test `ArgusLensAppViewModelTest.appViewModel_defersRestoredSelectedConversationUntilHydrationValidatesThreads` ensured selected conversation is exposed only after hydration validation; green after introducing token-free `AppRestorableEntryContext`, `SavedStateHandle` account/route/conversation-id persistence, hydration-time same-account/thread validation, tokenless/stale restore clearing, and hydration-only selected conversation restore; targeted `:app:testDebugUnitTest --tests "com.kzzz3.argus.lens.app.AppInitialHydrationRuntimeTest" --tests "com.kzzz3.argus.lens.app.SessionBoundaryTest" --tests "com.kzzz3.argus.lens.app.ArgusLensAppViewModelTest"`; full `testDebugUnitTest`; `lint`; `assembleDebug`; Kotlin LSP unavailable because `kotlin-lsp` is not installed; Markdown LSP unavailable | PASS |
| 2026-04-28 | P4 ViewModel restore test fixture hardening | Added direct JVM `ArgusLensAppViewModel` tests with handwritten app dependency fakes for `SavedStateHandle` chat restore persistence, non-chat/session-clear removal, and pre-hydration selection deferral; targeted `:app:testDebugUnitTest --tests "com.kzzz3.argus.lens.app.ArgusLensAppViewModelTest"`; full `testDebugUnitTest`; `lint`; `assembleDebug`; Kotlin LSP unavailable because `kotlin-lsp` is not installed; Markdown LSP unavailable | PASS |
| 2026-04-28 | P4 navigation mapping test hardening | Added direct JVM `TopLevelDestination.fromRoute` / `fromShellDestination` coverage for main shell tabs and secondary/non-top-level fallbacks; targeted `:app:testDebugUnitTest --tests "com.kzzz3.argus.lens.app.AppRouteNavigationRuntimeTest"`; full `testDebugUnitTest`; `lint`; `assembleDebug`; Kotlin LSP unavailable because `kotlin-lsp` is not installed; Markdown LSP unavailable | PASS |
| 2026-04-28 | P4 route action binding test hardening | Added direct JVM `AppRouteActionBindingsTest` coverage for auth entry callbacks, top-level/shell wallet routing, inbox conversation open/sync callbacks, and sign-out session-boundary delegation; targeted `:app:testDebugUnitTest --tests "com.kzzz3.argus.lens.app.AppRouteActionBindingsTest"`; full `testDebugUnitTest`; `lint`; `assembleDebug`; Kotlin LSP unavailable because `kotlin-lsp` is not installed; Markdown LSP unavailable | PASS |
| 2026-04-28 | P4 ViewModel realtime/status mutation hardening | Added direct JVM `ArgusLensAppViewModel` tests for realtime connection/event/generation mutations, chat status set/clear, and friend-request status snapshot/reset behavior; targeted `:app:testDebugUnitTest --tests "com.kzzz3.argus.lens.app.ArgusLensAppViewModelTest"`; full `testDebugUnitTest`; `lint`; `assembleDebug`; Kotlin LSP unavailable because `kotlin-lsp` is not installed; Markdown LSP unavailable | PASS |
| 2026-04-28 | P4 realtime action binding test hardening | Extended direct JVM `AppRouteActionBindingsTest` coverage to `realtimeConnectionCallbacks`, including connection state, event-id reset/recording, conversation thread publication, reconnect generation, and sign-out message delegation; targeted `:app:testDebugUnitTest --tests "com.kzzz3.argus.lens.app.AppRouteActionBindingsTest"`; full `testDebugUnitTest`; `lint`; `assembleDebug`; Kotlin LSP unavailable because `kotlin-lsp` is not installed; Markdown LSP unavailable | PASS |
