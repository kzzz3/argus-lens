...
internal class SessionBoundaryHandler(
    private val appShellUseCases: AppShellUseCases,
    private val refreshSessionOnce: suspend (AppSessionState, (AppSessionState) -> Unit) -> SessionRefreshOutcome,
    ...
    suspend fun applySuccessfulAuthResult(
        ...
        cancelCallSession()
        val signedInState = appShellUseCases.handleSignedIn(authenticatedSession)
    ...
    fun signOutToEntry(
        ...
        val signedOutState = appShellUseCases.createSignedOutState(
    ...
    suspend fun applySessionBoundary(
        ...
        if (session.isAuthenticated) {
            callbacks.onHydratedConversationAccountChanged(null)
            val signedInState = appShellUseCases.handleSignedIn(session)
