...
The current Lens implementation is an Android Studio-centered core-modular baseline: `:app`, `:feature`, `:core:data`, `:core:network`, `:core:database`, `:core:datastore`, `:core:model`, `:core:session`, and `:core:ui`. The durable target is an `:app` composition shell supported by shared `:core:*` modules and independently owned `:feature:*` modules.
